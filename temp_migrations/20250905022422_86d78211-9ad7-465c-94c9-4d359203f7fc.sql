...
-- Temporarily remove the NOT NULL constraint from storage_vicinity
ALTER TABLE public.parts 
ALTER COLUMN storage_vicinity DROP NOT NULL;
...
BEGIN
    ...
    -- If still no match, try partial match (common variations)
    IF structure_id IS NULL THEN
        SELECT id INTO structure_id
        FROM tools 
        WHERE category IN ('Infrastructure', 'Container') 
...
SET storage_vicinity = (
    SELECT map_vicinity_name_to_structure_id(storage_vicinity)::TEXT
)

...
async function listAllMembersWithStatus() {
  console.log('🔍 Listing ALL organization members (active and inactive)...');
  ...
  try {
    ...
    if (data.rows && data.rows.length > 0) {
      const active = data.rows.filter(row => row.is_active === true);
      const inactive = data.rows.filter(row => row.is_active === false);

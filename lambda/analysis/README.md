...
### 1. Deploy Lambda with Layer

```bash
cd /path/to/clever-widget-factory
./scripts/deploy/deploy-lambda-with-layer.sh action-scoring cwf-action-scoring
```

### 2. Wire to API Gateway

```bash
cd lambda/action-scoring
./wire-api-gateway.sh
```
...
curl -X POST https://0720au267k.execute-api.us-west-2.amazonaws.com/prod/api/action-scoring/generate \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer YOUR_TOKEN' \
  -d '{"prompt": "Score this action...", "model": "haiku"}'

# Test analyses endpoint
curl https://0720au267k.execute-api.us-west-2.amazonaws.com/prod/api/action-scoring/analyses \
  -H 'Authorization: Bearer YOUR_TOKEN'
```

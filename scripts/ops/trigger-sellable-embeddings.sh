...
echo "🔄 Triggering embedding generation for sellable products..."
echo ""
...
for PRODUCT in "${PRODUCTS[@]}"; do
  ...
  # Call regenerate endpoint with proper API Gateway event structure
  BODY=$(jq -n \
    --arg type "part" \
    --arg id "$PART_ID" \
  ...
  aws lambda invoke \
    ...
    /tmp/regen-$PART_ID.json > /dev/null 2>&1
  
  STATUS=$(cat /tmp/regen-$PART_ID.json | jq -r '.statusCode' 2>/dev/null || echo "error")
  ...
  if [ "$STATUS" = "200" ]; then
    ...
    echo "   ❌ Failed"
    cat /tmp/regen-$PART_ID.json | jq '.' 2>/dev/null || cat /tmp/regen-$PART_ID.json
    ((FAILED++))
  fi

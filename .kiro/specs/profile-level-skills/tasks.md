...
- [x] 1. Create Progression_Model pure function module
  - [x] 1.1 Create `lambda/learning/progressionModel.js` with `computeBloomLevel` and `computeTaperingDecision`
...
- [x] 2. Create profile skill state text utility functions
  - [x] 2.1 Add `composeProfileSkillStateText` and `parseProfileSkillStateText` to `lambda/skill-profile/index.js`
...
- [x] 3. Checkpoint — Verify pure functions and utilities
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Add profile skill API endpoints to Skill-Profile Lambda
  - [x] 4.1 Add `GET /api/profile-skills` handler in `lambda/skill-profile/index.js`
  ...
  - [x] 4.2 Add `POST /api/profile-skills/generate` handler in `lambda/skill-profile/index.js`
  ...
  - [x] 4.3 Add `POST /api/profile-skills/approve` handler in `lambda/skill-profile/index.js`
  ...
  - [x] 4.4 Add `PUT /api/profile-skills/:id/toggle` handler in `lambda/skill-profile/index.js`
  ...
  - [x] 4.5 Add `DELETE /api/profile-skills/:id` handler in `lambda/skill-profile/index.js`
    - Extract state ID from path
  ...
  - [x] 4.6 Wire new routes in the lambda handler's routing logic
...
- [x] 5. Checkpoint — Verify profile skill API endpoints
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. Add quiz generation integration in Learning Lambda
  - [x] 6.1 Add `fetchActiveProfileSkills` helper in `lambda/learning/index.js`
  ...
  - [x] 6.2 Add `buildProfileSkillPromptBlock` function in `lambda/learning/index.js`
  ...
  - [x] 6.4 Integrate profile skill context into `handleQuizGenerate`
  ...
  - [x] 6.5 Update `generateQuizViaBedrock` and `generateOpenFormQuizViaBedrock` to accept and include `profileSkillBlock`
...
- [x] 7. Checkpoint — Verify quiz generation integration
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. Add progression update after quiz evaluation in Learning Lambda
  - [x] 8.1 Add `updateProfileSkillProgression` function in `lambda/learning/index.js`
  ...
  - [x] 8.2 Call `updateProfileSkillProgression` from `handleEvaluate` after successful evaluation
...
- [x] 9. Checkpoint — Verify progression update integration
  - Ensure all tests pass, ask the user if questions arise.

- [x] 10. Create frontend hooks for profile skills
  - [x] 10.1 Create `src/hooks/useProfileSkills.ts` with TanStack Query hooks
...
- [x] 11. Create ProfileSkillsSection component and subcomponents
  - [x] 11.1 Create `src/components/ProfileSkillsSection.tsx` with ProfileSkillCard and CreateProfileSkillDialog
  ...
  - [x] 11.2 Create ProfileAxisDisplay subcomponent for bloom level visualization
...
- [x] 12. Replace ProfileIntentsSection with ProfileSkillsSection on profile settings page
  - [x] 12.1 Update `src/pages/Organization.tsx` to render ProfileSkillsSection
...
- [x] 13. Checkpoint — Verify all frontend changes
  - Ensure all tests pass, ask the user if questions arise.

- [x] 14. Deploy all changes
  - [x] 14.1 Deploy modified Lambdas using `deploy-lambda-with-layer.sh`
    - Deploy `lambda/skill-profile` → `cwf-skill-profile-lambda`
    - Deploy `lambda/learning` → `cwf-learning-lambda`
    - _Requirements: all_

  - [x] 14.2 Add API Gateway routes for profile skill endpoints

...
- [ ] 1. Database schema migration and embedding composition update
  - [ ] 1.1 Run database migration to add `expected_state` and `skill_profile` columns to the `actions` table
  ...
  - [ ] 1.2 Update `composeActionEmbeddingSource` in `lambda/shared/embedding-composition.js` to include `expected_state`
  ...
  - [ ] 1.4 Update `cwf-actions-lambda` to handle `expected_state` in create and update operations
...
- [ ] 2. Checkpoint — Verify schema migration and embedding composition
  - Ensure migration ran successfully (query `SELECT expected_state, skill_profile FROM actions LIMIT 1`)
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 3. Skill Profile Lambda (`cwf-skill-profile-lambda`)
  - [ ] 3.1 Create the Lambda function directory and handler scaffold
  ...
  - [ ] 3.2 Implement the skill profile generation endpoint (`POST /api/skill-profiles/generate`)
  ...
  - [ ] 3.3 Implement the skill profile approval endpoint (`POST /api/skill-profiles/approve`)
  ...
  - [ ] 3.4 Implement the skill profile deletion endpoint (`DELETE /api/skill-profiles/:actionId`)
...
- [ ] 4. Deploy Skill Profile Lambda and add API endpoints
...
- [ ] 5. Checkpoint — Verify Skill Profile Lambda
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. Capability Lambda (`cwf-capability-lambda`)
  - [ ] 6.1 Create the Lambda function directory and handler scaffold
  ...
  - [ ] 6.2 Implement the individual capability assessment endpoint (`GET /api/capability/:actionId/:userId`)
    ...
    - Perform vector similarity search in `unified_embeddings` for `action_observation` entries similar to the skill profile embedding, scoped by `organization_id`
  ...
  - [ ] 6.3 Implement the organization capability assessment endpoint (`GET /api/capability/:actionId/organization`)
  ...
  - [ ] 6.4 Implement shared utility functions for capability computation
...
- [ ] 7. Deploy Capability Lambda and add API endpoints
...
- [ ] 8. Checkpoint — Verify Capability Lambda
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Extend embedding pipeline for `action_observation` entity type
  - [ ] 9.1 Update `lambda/states/index.js` to send additional `action_observation` SQS message
    - In the `resolveAndQueueEmbedding` function, after sending the existing `state` embedding message, check if the state has any `state_links` with `entity_type = 'action'`
    - If so, send an additional SQS message with `entity_type: 'action_observation'`, `entity_id: state.id`, using the same `embedding_source`
    - This is fire-and-forget, consistent with existing pattern
    - _Requirements: 5.1, 5.2_

  - [ ]* 9.2 Write property test for observation creates `action_observation` embedding (Property 11)
    - **Property 11: Observation creates action_observation embedding**
    - Verify that for any state linked to an action, an SQS message with `entity_type: 'action_observation'` is sent in addition to the `state` message
    - **Validates: Requirements 5.1**

- [ ] 10. Checkpoint — Verify embedding pipeline extension
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 11. Frontend: `expected_state` field in UnifiedActionDialog
  - [ ] 11.1 Add `expected_state` to the `BaseAction` interface in `src/types/actions.ts`
  ...
  - [ ] 11.2 Add the `ExpectedStateField` to `src/components/UnifiedActionDialog.tsx`
...
- [ ] 12. Frontend: Skill Profile Panel
  - [ ] 12.1 Create `src/hooks/useSkillProfile.ts` with TanStack Query hooks
  ...
  - [ ] 12.2 Create `src/components/SkillProfilePanel.tsx`
  ...
  - [ ] 12.3 Integrate `SkillProfilePanel` into the action detail view
...
- [ ] 13. Frontend: Radar Chart and Capability Assessment
  - [ ] 13.1 Install Recharts and create `src/hooks/useCapability.ts`
  ...
  - [ ] 13.2 Create `src/components/RadarChart.tsx`
  ...
  - [ ] 13.3 Create `src/components/AxisDrilldown.tsx`
  ...
  - [ ] 13.4 Create `src/components/CapabilityAssessment.tsx` container component
  ...
  - [ ] 13.5 Integrate `CapabilityAssessment` into the action detail view
...
- [ ] 14. Checkpoint — Verify all frontend components
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 15. End-to-end wiring and final integration
  - [ ] 15.1 Wire the `expected_state` AI generation to the existing Maxwell AI pattern
  ...
  - [ ] 15.2 Redeploy `cwf-actions-lambda` with `expected_state` changes
    - Deploy using `./scripts/deploy/deploy-lambda-with-layer.sh actions cwf-actions-lambda`
    - _Requirements: 1.1, 1.6_

  - [ ] 15.3 Redeploy `cwf-states-lambda` with `action_observation` embedding changes
    - Deploy the updated states Lambda that sends the additional `action_observation` SQS message
    - _Requirements: 5.1_

  - [ ] 15.4 Redeploy the common Lambda layer with updated `embedding-composition.js`
...
- The existing embedding pipeline (SQS + `cwf-embeddings-processor`) handles the new `action_skill_profile` and `action_observation` entity types without modification — only the SQS message senders need updating

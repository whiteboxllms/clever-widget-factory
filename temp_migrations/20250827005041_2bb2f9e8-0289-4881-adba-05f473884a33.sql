...
-- Create a unique partial index to enforce one active checkout per tool
CREATE UNIQUE INDEX idx_unique_active_checkout_per_tool 
...
-- Step 4: Create trigger to validate checkouts
CREATE TRIGGER trigger_validate_checkout
  BEFORE INSERT ON checkouts
  FOR EACH ROW
  EXECUTE FUNCTION validate_checkout_operation();
 ...
 * @param {string} entityType - Entity type (part, tool, action, issue, policy, skill_axis, etc.)
 * @param {string|null} entityId - Entity UUID (null for skill_axis — DB generates via gen_random_uuid())
 ...
 * @param {string|null} [actionId] - Action UUID (required for skill_axis rows)
 * @param {string|null} [axisKey] - Axis key string (required for skill_axis rows)
 */
async function writeToUnifiedTable(entityType, entityId, embeddingSource, embedding, organizationId, actionId = null, axisKey = null) {
  ...
  if (entityType === 'skill_axis' && actionId && axisKey) {
    // skill_axis rows: entity_id is generated by the DB (gen_random_uuid()).
    // Conflict target is (entity_type, action_id, axis_key) since entity_id is not stable.
    const escapedActionId = actionId.replace(/'/g, "''");
    const escapedAxisKey = axisKey.replace(/'/g, "''");
    ...
    // All other entity types: standard upsert keyed on (entity_type, entity_id, model_version).
...
exports.handler = async (event) => {
  ...
  for (const record of event.Records) {
    try {
      ...
      let { entity_type, entity_id, embedding_source, organization_id, fields, assets, action_id, axis_key } = message;
      ...
      if (WRITE_TO_UNIFIED) {
        ...
        await writeToUnifiedTable(entity_type, entity_id, embedding_source, embedding, organization_id, action_id, axis_key);
      }
